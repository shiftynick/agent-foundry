...
test("Cursor isolated writes do not claim sandboxing on Windows", async () => {
  let captured;
  await runAgent(
    { provider: "cursor", prompt: "edit", cwd: root, model: "cursor-grok-4.6-medium", access: "edit-isolated" },
    { execute: async (invocation) => {
      captured = invocation;
...
test("a banner line before the stream does not fail an otherwise successful run", async () => {
  const result = await runAgent(
    { ...cursorEvents, model: "cursor-grok-4.6-medium" },
...
test("output that cannot be read is unparsed, not failed", async () => {
  const result = await runAgent({ ...cursorEvents, model: "cursor-grok-4.6-medium" }, stub("banner one\nbanner two"));
...
test("the last terminal marker decides the verdict, in both directions", async () => {
  ...
  const afterSuccess = await runAgent({ ...cursorEvents, model: "cursor-grok-4.6-medium" }, stub(`${success}\n${error}`));
  assert.equal(afterSuccess.status, "failed", "an error after a success must not be swallowed");

  const retried = await runAgent({ ...cursorEvents, model: "cursor-grok-4.6-medium" }, stub(`${error}\n${success}`));
...
test("a non-zero exit is still a plain failure", async () => {
  const result = await runAgent({ ...cursorEvents, model: "cursor-grok-4.6-medium" }, stub("", { exitCode: 1 }));
...
test("Cursor defaults its model and says so", async () => {
  ...
  assert.equal(captured.args[index + 1], "cursor-grok-4.6-medium");
  assert.equal(defaulted.modelDefaulted, true);

  const explicit = await runAgent({ ...cursorEvents, model: "cursor-grok-4.6-medium" }, withCapture);
...
test("supported models lists are curated per provider", async () => {
  ...
  assert.equal(SUPPORTED_MODELS.cursor.includes("cursor-grok-4.6-medium-fast"), false);
...
test("off-allowlist and Grok-fast Cursor models fail closed", async () => {
  ...
  await assert.rejects(
    () => runAgent({ provider: "cursor", prompt: "x", cwd: root, model: "cursor-grok-4.6-high-fast" }, stub(providerOutput("cursor"))),
